...
class Tree extends Dbp
{
    ...
    protected function _initInactiveCategoryIds()
    {
        ...
        $this->_eventManager->dispatch('blog_category_tree_init_inactive_category_ids', ['tree' => $this]);
        return $this;
    }
    ...
    public function loadByIds($ids, $addCollectionData = true, $updateAnchorProductCount = true)
    {
        ...
        if (empty($ids)) {
            $select = $this->_conn->select()->from($this->_table, 'category_id')->where($levelField . ' <= 2');
        ...
        $select = $this->_conn->select()->from($this->_table, ['path', 'level'])->where('category_id IN (?)', $ids);
    ...
    {
        $meta = $this->getMetadataPool()->getMetadata(CategoryManagementInterface::class);
        ...
        $categoriesTable = $this->_coreResource->getTableName('magefan_blog_category');
        $categoriesProductsTable = $this->_coreResource->getTableName('magefan_blog_post');
